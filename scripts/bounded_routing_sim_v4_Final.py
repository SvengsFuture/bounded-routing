...
def update_pair_assertions(
    ...
    pre_execution_record_stream_hash: str,
) -> None:
    ...
    replay_structural_df = pd.read_csv(structural_manifest_path, float_precision="round_trip")
    replay_records_df = build_shape_records(replay_structural_df)
    replay_hash = dataframe_content_hash(replay_records_df)
    ledger.check(
        "A7",
        replay_hash == pre_execution_record_stream_hash
        and replay_records_df.record_payload_hash.tolist() == records_df.record_payload_hash.tolist(),
        f"{tag}: saved structural manifest replays to byte-identical shape records",
    )
    ...
    post_records_hash = dataframe_content_hash(records_df)
    gate_records_hash = dataframe_content_hash(pd.DataFrame(gate.records, columns=records_df.columns))
    ledger.check(
        "A25",
        post_records_hash == pre_execution_record_stream_hash
        and gate_records_hash == pre_execution_record_stream_hash,
...
def dataframe_content_hash(df: pd.DataFrame) -> str:
    """Deterministic hash for an in-memory dataframe stream."""
...
def main() -> int:
    ...
    bundle: dict[tuple[str, int], tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame, Path, str]] = {}
    manifest_hashes: dict[str, str] = {}
    for scenario in scenarios:
        for seed in seeds:
            ...
            pre_execution_record_stream_hash = dataframe_content_hash(records_df)
            bundle[(scenario.scenario_id, seed)] = (
                task_df, structural_df, deformation_df, truth_df, records_df,
                structural_path, pre_execution_record_stream_hash,
            )
    ...
    for scenario in scenarios:
        for seed in seeds:
            ...
            update_pair_assertions(
                ledger, scenario, seed, task_df, structural_df, records_df, rows_df, gate,
                manifests_saved, structural_path, pre_execution_record_stream_hash,
            )
